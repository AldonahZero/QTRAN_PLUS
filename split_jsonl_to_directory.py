...
def split_jsonl_to_directory(
    input_file: str, output_dir: str = None, verbose: bool = True
):
    """
    ...
    with open(input_path, "r", encoding="utf-8") as f:
        for line_number, line in enumerate(f, start=1):
            ...
            if "index" in record:
                ...
                if verbose:
                    print(
                        f"警告: 第 {line_number} 行缺少 'index' 字段，使用行号 {index}"
                    )
...
def main():
    ...
    if len(sys.argv) < 2:
        print(
            "用法: python split_jsonl_to_directory.py <input_jsonl_path> [output_dir]"
        )
        print("\n示例:")
        print("  python split_jsonl_to_directory.py Input/demo1.jsonl")
        print(
            "  python split_jsonl_to_directory.py Input/demo1.jsonl Input/demo1_output"
        )
        sys.exit(1)
