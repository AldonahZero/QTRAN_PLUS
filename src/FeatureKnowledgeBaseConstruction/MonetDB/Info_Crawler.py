...
def get_table_column_contents(soup_tbody):
    ...
    for tr in trs:
        info_contents = (
            []
        )  # 内部存储多个[]格式的信息，因为feature，description和example都可能不止一个
        # 获取tr内的所有td，处理
...
def function_crawler(origin_category, title, html, func_dic_filename, op_dic_dicname):
    result = {}
    table_include = ["Function", "Operator", "Statistic Function"]
    timeout = 5  # 等待时间
    options = set_options()
    ...
    for table in soup_tables:
        ...
        for name_index in range(len(column_names)):
            if column_names[name_index] in [
                "Function",
                "Operator",
                "Statistic Function",
            ]:
                feature_column_indexes.append(name_index)
            elif column_names[name_index] in [
                "Return type",
                "Argument types",
                "Description",
            ]:
    ...
    for key, value in functions_dic.items():
        ...
        if os.path.exists(os.path.join(func_dic_filename, filename)):
            print(filename + ":已经存在")
        with open(
            os.path.join(func_dic_filename, filename), "w", encoding="utf-8"
        ) as w:
...
def data_types_crawler(category_key, statement_key, statement_value, dic_filename):
    ...
    for table in soup_tables:
        ...
        for content in table_contents:
            ...
            with open(
                os.path.join(dic_filename, str(file_cnt) + ".json"),
                "w",
                encoding="utf-8",
            ) as w:
...
def crawler_results(
    feature_type, func_htmls_filename, func_dic_filename, op_dir_dicname
):
        ...
        for category_key, value in html_contents.items():
            for statement_key, statement_value in value.items():
                ...
                if feature_type in ["function", "operator"]:
                    ...
                    data_types_crawler(
                        category_key, statement_key, statement_value, func_dic_filename
                    )
                print("----------------------")
