...
def category_classifier(results_dicname, results_category_dicname):
    ...
    for json_file in json_files:
        ...
        for category in data["Category"]:
            with open(
                os.path.join(results_category_dicname, category + ".jsonl"),
                "a",
                encoding="utf-8",
            ) as w:
                json.dump(data, w)
...
def processor(source_dic_name, direcct_din_name, direcct_din_name_category):
    ...
    for file in filenames:
        ...
        for key, value in contents.items():
            ...
            direcct_filaname = os.path.join(
                direcct_din_name,
                str(file_cnt) + "_" + sanitize_title(new_value["Feature"][0]) + ".json",
            )
            ...
            for category in new_value["Category"]:
                with open(
                    os.path.join(direcct_din_name_category, category + ".jsonl"),
                    "a",
                    encoding="utf-8",
                ) as w:
...
function_direct_category = (
    "../../../FeatureKnowledgeBase/sqlite/function/results_category"
)
...
statement_source = (
    "../../../Feature Knowledge Base_PINOLO_WithoutProcess/sqlite/statement"
)
statement_direct = (
    "../../../FeatureKnowledgeBase/sqlite/SQL_Statements/SQL_Statements_Results"
)
