...
def potential_dialect_features_process_and_map(
    tool,
    a_db,
    b_db,
    source_filename,
    ...
    * 统计：针对a_db的所有测试语句，potential dialect features/总features个数
    ...
    a_merge_feature_filename = os.path.join(
        "..",
        "..",
        "FeatureKnowledgeBase Processed1",
        a_db,
        "RAG_Embedding_Data",
        names + ".jsonl",
    )
    ...
    features_mapping_filename = os.path.join(
        "..",
        "..",
        "RAG_Feature_Mapping",
        a_db,
    ...
    for content in contents:
        ...
        if feature_types[0] == "function":
            feature_indexes = content[
                "SqlPotentialFunctionIndexes"
            ]  # potential functions的分词的下标
        ...
        for index in feature_indexes:
            ...
            for line in a_merge_features:
                ...
                if (
                    str(feature_name).lower().strip()
                    in a_feature_Feature.split("(")[0].lower().strip()
                ):
            ...
            if feature_searched_index:
                ...
                feature_searched_content = json.loads(
                    a_merge_features[feature_searched_index]
                )
                # 选择最短的sql作为方言检测sql
                ...
                if len(effective_sqls):
                    ...
                    exec_result, exec_time, error_message = exec_sql_statement(
                        tool, b_db, dialect_recognize_sql
                    )
                    ...
                    if exec_result == None:
                        ...
                        potential_dialect_mapping_index.append(
                            [
                                feature_searched_index,
                                json.loads(mapping_info[feature_searched_index])[
                                    "b_db"
                                ]["index"],
                            ]
                        )
        ...
        if feature_types[0] == "function":
            ...
            content["SqlPotentialDialectFunctionMapping"] = (
                potential_dialect_mapping_index
            )
        elif feature_types[0] == "operator":
            content["SqlPotentialDialectOperator"] = potential_dialect
            content["SqlNotDialectOperator"] = not_potential_dialect
            content["SqlPotentialDialectOperatorMapping"] = (
                potential_dialect_mapping_index
            )
        else:
            pass
...
def sqlancer_potential_dialect_features_process_and_map(
    a_db, b_db, potential_dialect, feature_type, search_k=0, version_id=1
):
    """
    ...
    for dialect in potential_dialect:
        ...
        for info in mapping_info:
            ...
            if a_feature_str.startswith(dialect_str + "("):
                # 为方言添加对应的mapping index
                potential_dialect_mapping_index.append(
                    [map["a_db"]["index"], map["b_db"]["index"]]
                )
                break
    return potential_dialect_mapping_index


def potential_dialect_features_recognizer(
    a_db,
    b_db,
    source_filename,
    dir_filename,
    ...
    potential_dialect_features_process_and_map(
        a_db,
        b_db,
        source_filename,
        dir_filename,
