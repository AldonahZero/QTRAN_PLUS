...
def fine_tune_mutation_llm(api_key, training_data_filename, suffix):
    ...
    training_file_object = client.files.create(
        file=open(training_data_filename, "rb"), purpose="fine-tune"
    )
    ...
    print(
        'Please record the "Fine-tuning Job ID" to check the fine-tuning stage, which can take several minutes.\n'
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Fine-tune or Check fine-tuning status of Mutation LLM"
    )

    # 添加命令行参数
    parser.add_argument("--api_key", required=True, help="Your OpenAI API key")
    parser.add_argument(
        "--training_data_filename", help="Path to the training data file (e.g., .jsonl)"
    )
    ...
    if args.job_id:
        ...
        if not args.training_data_filename or not args.suffix:
            print(
                "You must provide --training_data_filename and --suffix for fine-tuning."
            )
        else:
            fine_tune_mutation_llm(
                args.api_key, args.training_data_filename, args.suffix
            )
